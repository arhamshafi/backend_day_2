...
function Comments() {
    ...
    useEffect(() => {
        axios.get("http://localhost:4505/comments").then((res) => setapi(res.data))
    }, [])

    return (
        <div className='w-full min-h-screen bg-white '>
            <Navbar />
            <h1 className='text-4xl text-black mt-5 font-bold text-center capitalize'>Comments aPI here !!</h1>
                    ...
                    api && api.map((ele, idx) => {
                        return (
                            ...
                            <div key={idx} className='w-[300px] h-[150px] cursor-pointer px-5 bg-green-600 rounded-xl mt-4 transition-all duration-150 ease-linear hover:scale-102 hover:bg-green-400'>
                                ...
                                <p className='text-white text-md '> {ele.name} </p>
                                <p className='text-white text-lg '> {ele.email} </p>
                            </div>
                        )
                    })
                }
